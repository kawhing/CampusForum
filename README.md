...
- **Docker部署** - 一键启动，开箱即用
- **心理援助引导** - 检测轻生/高风险关键词时自动弹窗联系方式，提供心理辅导页面入口，并对含脏话内容降低信誉分以保护社区氛围  
- **本地 AI 助手** - 由管理员配置 Ollama 本地模型，支持普通问答与心理支持模式  

### （已移除）文本安全/心理风险模型

应反馈要求，项目已取消附带的 AI 文本安全模型镜像，避免下载体积较大的模型依赖。现有关键词/信誉分规则仍可正常工作，如需自定义模型可自行扩展。

## 🤖 本地 AI 助手（Ollama）

本项目支持接入 **本地 Ollama 模型**，提供普通问答与心理支持两种主题模式。AI 设置由管理员在管理控制台中配置（无需容器部署）。
...
2. 拉取小参数模型，例如：
   ...
   ollama pull llama3
   ```
3. 在 `.env` / `backend/.env` 中配置：
   ```
   OLLAMA_BASE_URL=http://localhost:11434
   OLLAMA_MODEL=llama3
   OLLAMA_TIMEOUT_MS=20000
   ```
4. 管理员进入「管理控制台 → AI 设置」保存配置即可生效
